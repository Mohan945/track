...
sed -i '/^[[:space:]]*$/d' "$DB_LIST"   # Remove blank lines and spaces
tr -d '\r' < "$DB_LIST" > "$CLEAN_DB_LIST"  # Remove Windows ^M characters
...
while IFS= read -r DB_NAME || [ -n "$DB_NAME" ]; do
    # Debugging: Log the database name being processed
    echo "[$(date)] Processing database: [$DB_NAME]" | tee -a "$LOG_FILE"

    # Ensure DB_NAME is not empty
    if [ -z "$DB_NAME" ]; then
        echo "[$(date)] Skipping empty database entry." | tee -a "$LOG_FILE"
        continue
    fi
